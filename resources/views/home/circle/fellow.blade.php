...
<div id="mainbody">
  ...
  <div class="info">
        <div class="card" style="width: 100%;">
           <h1>好友列表</h1>
           <a href="/circle/fwllowask" class="cta" style="margin-left: 60px;">我的好友请求</a>
          <ul class="bloglist">
            @if(count($data) == 0)
              <h1 style="margin-left: 50px;margin-top: 50px;">您目前还没有好友哦~</h1>
            @else
            @foreach($data as $k => $v)
          <!-- 每一个好友请求 -->
                ...
                <div class="row_box">
                    <ul class="textinfo">
                        <div  style="margin-top: 10px;">
                            ...
                            <!-- 好友姓名结束 -->
                            <a href="/circle/delete/{{ $v->id }}" style="float: right;position: relative;top: -60px;right: 10px;height: 20px;" href="" class="cta">删除好友</a>
                        </div>
                    </ul>
                    <ul class="details">
                        <li class="icon_time"><a href="javascript:;">最后登录： @if($v->user->last_time) {{ date('Y年m月d日 H时i分s秒',$v->user->last_time) }} @else 暂无记录 @endif</a></li>
                    </ul>
                </div>
            </li>
            @endforeach
